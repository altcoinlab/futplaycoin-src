...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("Futplaycoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #FutplaycoinTEST2\r");
            Send(hSocket, "WHO #FutplaycoinTEST2\r");
        } else {
            // randomly join #Futplaycoin00-#Futplaycoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Futplaycoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Futplaycoin%02d\r", channel_number).c_str());
        }
